...
#voting a post user already voted on
...
#delete a vote that doesn't exist
def test_delete_vote_non_exist(authorized_client, test_posts):
    res = authorized_client.post("/vote/", json={"post_id": test_posts[3].id, "dir": 0})
    assert res.status_code == 404
    
#voting on post that doesn't exist
...
def test_vote_post_unathorized_user(client, test_posts):
